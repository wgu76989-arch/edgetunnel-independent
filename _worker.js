...
export default {
	async fetch(request, env, ctx) {
		...
		const 管理员密码 = env.ADMIN_SECRET || env.ADMIN || env.admin || env.PASSWORD || env.password || env.pswd || env.TOKEN || env.KEY || env.UUID || env.uuid;
		...
		if (访问路径 === 'version') {// 版本信息接口
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					const 订阅TOKEN = await MD5MD5(host + userID), 作为优选订阅生成器 = ['1', 'true'].includes(env.BEST_SUB) && url.searchParams.get('host') === 'example.com' && url.searchParams.get('uuid') === '00000000-0000-4000-8000-000000000000' && UA.toLowerCase().includes('tunnel (' + 自有GitHub项目.toLowerCase());
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							if (!config_JSON.订阅转换配置.SUBAPI) return new Response('订阅转换服务尚未配置，请在后台填写自行部署的 subconverter 地址。', { status: 501 });
							...
							try {
								const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edgetunnel (' + 自有GitHub项目 + ')' } });
								if (response.ok) {
		...
		const 当前连接任务 = (async () => {
			...
			try {
				if (使用木马反代) {
					...
					if (!ctx反代IP) {
						log(`[TCP直连] 未配置 PROXYIP，重新直连: ${host}:${portNum}`);
						newSocket = await connectDirect(host, portNum, 本次首包数据, true);
					} else {
	...
	try {
		...
		const 添加规则集 = (type, code) => {
			...
			if (!ruleSet.some(item => item?.tag === tag)) {
				...
				ruleSet.push({ tag, type: 'remote', format: 'binary', url: `https://raw.githubusercontent.com/wgu76989-arch/edgetunnel-sing-${type}/main/${tag}.srs`, ...(legacyOptions?.download_detour ? { download_detour: legacyOptions.download_detour } : {}) });
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const host = hostname, Ali_DoH = "https://dns.alidns.com/dns-query", ECH_SNI = "cloudflare-ech.com", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
		...
		订阅转换配置: {
			SUBAPI: '',
			SUBCONFIG: 'https://raw.githubusercontent.com/wgu76989-arch/edgetunnel-acl4ssr/main/Clash/config/ACL4SSR_Online_Mini_MultiMode_CF.ini',
			SUBEMOJI: false,
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	...
	const cidr_url = 运营商文件标识 === 'cf' ? 'https://raw.githubusercontent.com/wgu76989-arch/edgetunnel-data/main/CF-CIDR.txt' : `https://raw.githubusercontent.com/wgu76989-arch/edgetunnel-data/main/CF-CIDR/${运营商文件标识}.txt`;
...
async function 获取优选订阅生成器数据(优选订阅生成器HOST) {
	...
	try {
		const response = await fetch(优选订阅生成器URL, {
			headers: { 'User-Agent': 'v2rayN/edgetunnel (' + 自有GitHub项目 + ')' }
		});
